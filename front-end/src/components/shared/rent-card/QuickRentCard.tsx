...
export const QuickRentCard = ({ rentId }: Props) => {
  ...
  return (
    <div className="hidden lg:grid grid-cols-2 h-full ">
      <div className="relative text-white w-full  h-full p-5 flex flex-col  justify-between items-start ">
        <button
          onClick={() => {
            mutate({ id: rentId! });
            setIsLiked(!isLiked);
          }}
          className="h-fit absolute top-4 right-4 z-[10] "
        >
          <img src={isLiked ? HeartFilledImg : HeartOutlinedImg} alt="heart" />
        </button>
        <img
          className="absolute h-full w-full  bottom-0 left-0  object-cover"
          src="https://img.sixt.com/1600/6f09b0e8-6820-4ac0-bedd-5797e9814c18.jpg"
          alt=""
        />
        <RenderIf condition={isLoading}>
          <div className="absolute w-full h-full top-[50%] left-0  object-cover  translate-y-[-50%] bg-black bg-opacity-40 animate-pulse"></div>
        </RenderIf>
        <img
          className="absolute w-full  top-[50%] left-0  object-cover  translate-y-[-50%]"
          src={imageUrls?.[0]}
          alt=""
        />
        <div className="flex flex-col gap-2 justify-between h-full w-full z-[1] ">
          <div className="flex flex-col">
            ...
            <div className=" text-[14px] mt-1">{category?.title}</div>
          </div>

          <div className="w-full mb-2  flex flex-wrap items-center  justify-center">
      ...
      <div className="flex flex-col justify-between p-9">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-4">
            ...
            <div className="w-full gap-28 border-2  border-black  rounded-lg flex justify-between items-center p-3">
              <div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              <div
                ...
                } w-full cursor-pointer gap-28 border-2 rounded-b-none border-black  rounded-lg flex justify-between items-center p-3 `}
              >
                <div className="flex gap-4 items-center">
                  <div
                    className={`${
                      selectedOption === 1 ? "bg-black" : ""
                    } rounded-full w-6 h-6  flex items-center justify-center border border-gray-600`}
                  >
                  ...
                  <div>
                    <div
                      className={`${
                        isLoading
                          ? "animate-pulse  h-5 w-24 bg-black  rounded-md"
                          : ""
                      } font-bold `}
                    >
                      {days * Number(maxKmAllowedPerDay)}km
                    </div>
                    <div className="text-[12px] text-gray-600">
                      + {currency == "USD" ? "$" : "₼"}
              ...
              <div
                ...
                } w-full cursor-pointer gap-28 border-2 rounded-t-none border-t-0 border-black  rounded-lg flex justify-between items-center p-3 `}
              >
                <div className="flex gap-4 items-center">
                  <div
                    className={`${
...
export default function FormattedPrice({
  ...
  const [dollar, cents] = price.toFixed(2).split(".");

  return (
    <div
      className={`${
        isLoading ? "animate-pulse  h-8  bg-black  rounded-md !text-black " : ""
      }   flex items-end`}
    >
      <span className="text-lg font-bold  leading-[20px]">
        {currency == "USD" ? "$" : "₼"}
      </span>
      <span className="text-3xl font-bold leading-[30px] ">{dollar}</span>
      <span className="text-lg font-bold leading-[20px]">.{cents}</span>
      <RenderIf condition={showDay}>
        <span>/day</span>
      </RenderIf>
    </div>
  );
